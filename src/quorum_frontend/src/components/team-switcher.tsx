...
export function TeamSwitcher({ user, organizations }: { user: UserData, organizations: Organization[] }) {
	const { isMobile, setTeamId } = useSidebar();
    ...
    const [activeTeam, setActiveTeam] = React.useState<UserData | Organization>(() => {
		return globals.teamId === "user"
			? user
			: organizations.find((organization) => organization.name.replace(/\s+/g, '+') === globals.teamId) || user;
	});
	...
	function changeActive(team: Organization | UserData, view: "user" | "org") {
		const teamId =
			view === "user" ? "user" : "isPublic" in team ? team.name.replace(/\s+/g, '+') : "user";
console.log(team)
		...
		if (view === "org" && "isPublic" in team) {
				navigate(`/dashboard/organizations/${team.name.replace(/\s+/g, '+')}`);
		} else if (view === "user") {
			navigate(`/dashboard/overview`);
		}  
        console.log(teamId, globals)	}
					...
					<DropdownMenuTrigger asChild>
						<SidebarMenuButton
							...
							<div className='grid flex-1 text-left text-sm leading-tight'>
								...
								<span className='truncate text-xs'>{"isPublic" in activeTeam ? activeTeam.description.slice(0, 6) : activeTeam.principalId.slice(0, 6)}...</span>
							</div>
